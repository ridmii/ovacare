...
import { ArrowLeft, ArrowRight, Eye, EyeOff, Lock, Mail, Sparkles, User, ShieldCheck, HeartPulse, AlertCircle, CheckCircle, Loader2 } from 'lucide-react'
import { GlassCard } from '../components/GlassCard'
import { GradientButton } from '../components/GradientButton'
import { FloatingElements } from '../components/FloatingElements'
import { signup, googleAuth } from '../utils/authService'
...
export function SignupPage({ setActivePage }: SignupPageProps) {
  const { t } = useTranslation()
  const { setUser } = useAuth()
  const [showPassword, setShowPassword] = useState(false)
  const [form, setForm] = useState({ name: '', email: '', password: '', agreedToTerms: false })
  ...
  const handleGoogleSignup = useGoogleLogin({
    onSuccess: async (tokenResponse) => {
      ...
      try {
        ...
        // Use the id_token approach via credential flow is better, but with implicit flow
        // we call our backend with the access_token to verify
        const userInfo = await userInfoRes.json()
        // For implicit flow, we pass the access_token; backend uses google-auth-library differently
        // Use a workaround: sign in via access_token passed as credential
        const res = await fetch(`${process.env.REACT_APP_API_URL || 'http://127.0.0.1:5000'}/api/auth/google-access-token`, {
          method: 'POST',
        ...
        const data = await res.json()
        const { saveSession } = await import('../utils/authService')
        ...
        >
          ...
          <div className="hidden lg:flex lg:flex-col lg:justify-between lg:rounded-[2rem] lg:border lg:border-white/40 lg:bg-white/40 lg:p-10 lg:backdrop-blur-xl lg:shadow-[0_20px_80px_rgba(110,86,207,0.14)]">
            ...
            <div className="grid gap-4 sm:grid-cols-3">
              {(Array.isArray(t('auth.signup.features', { returnObjects: true })) 
                ? (t('auth.signup.features', { returnObjects: true }) as any[]) 
                : [
          ...
          <GlassCard className="relative mx-auto w-full max-w-[400px] p-8" glow>
            <div className="text-center">
              ...
              <h2 className="text-2xl font-bold text-ovacare-navy">{t('auth.signup.title')}</h2>
            ...
            <div className="relative my-4 flex items-center">
              ...
              <span className="mx-4 text-xs font-medium text-ovacare-gray">{t('auth.signup.orEmail')}</span>
              <div className="flex-1 border-t border-gray-200" />
            </div>
            ...
            <form className="space-y-3" onSubmit={handleSubmit}>
              <div>
                <label className="mb-1 block text-sm font-medium text-ovacare-navy">{t('auth.signup.fullNameLabel')}</label>
              ...
              <div>
                <label className="mb-1 block text-sm font-medium text-ovacare-navy">{t('auth.signup.emailLabel')}</label>
                <div className="relative">
              ...
              <div>
                <label className="mb-1 block text-sm font-medium text-ovacare-navy">{t('auth.signup.passwordLabel')}</label>
                <div className="relative">
              ...
              <div className="rounded-2xl border border-white/70 bg-white/70 p-4 text-sm text-ovacare-gray">
                {t('auth.signup.privacyNote')}
              </div>

              <div className="flex items-start gap-3 text-sm text-ovacare-gray">
                <input
                  type="checkbox"
                  name="agreedToTerms"
                  checked={form.agreedToTerms}
                  onChange={handleChange}
                  className="mt-1 rounded border-gray-300 text-ovacare-purple focus:ring-ovacare-purple"
                />
                <span>{t('auth.signup.agreeTerms')}</span>
              </div>

              <GradientButton type="submit" className="mt-4 w-full py-2.5" disabled={loading}>
                {loading ? <Loader2 className="mx-auto h-5 w-5 animate-spin" /> : t('auth.signup.submitButton')}
              </GradientButton>
            </form>
            ...
            <div className="mt-5 text-center">
              <p className="text-sm text-ovacare-gray">
                ...
                >
                  {t('auth.signup.loginLink')} <ArrowRight className="h-4 w-4" />
                </button>
              </p>
            </div>
          </GlassCard>
